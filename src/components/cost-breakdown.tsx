    ...
    <div className="bg-[#1a1a1a] text-[#f2f2f2] rounded-lg overflow-hidden shadow-2xl">
      ...
      {persistedId && version != null && (
        <div className="px-6 pt-4 flex items-center gap-3">
          <span className="bg-[#ccff00] text-black py-1 px-2 text-xs font-bold uppercase tracking-widest rounded">
          ...
          <button
            ...
            className="text-xs font-bold text-[#ccff00] uppercase tracking-wider hover:underline cursor-pointer"
          >
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-bold tracking-wider text-[#ccff00] uppercase font-headline">
          ...
          <div className="bg-white/5 p-4 rounded-lg border border-white/5 min-w-[240px] backdrop-blur-sm">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-xs font-bold text-[#ccff00] font-mono tabular-nums">
            ...
            <p className="text-xs mt-2 text-[#999] opacity-60 leading-tight">
      ...
      <div className="bg-white/5 px-4 sm:px-6 py-3 flex flex-wrap gap-3 sm:gap-6 border-b border-white/5">
        {estimate.assumptions.map((a) => (
          <div key={a.field} className="flex items-center gap-2 max-w-[280px] min-w-0">
            <span className="text-xs font-bold text-[#999] uppercase tracking-wider shrink-0">{a.label}:</span>
        ...
        <div className="ml-auto flex items-center gap-2">
          <span className="text-xs font-bold text-[#999]/60 uppercase tracking-wider">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <thead>
            <tr className="bg-white/5">
              <th className="px-6 py-4 text-xs font-bold text-[#999] uppercase tracking-wider">
                {t("costBreakdown.category")}
              </th>
              <th className="px-6 py-4 text-xs font-bold text-[#999] uppercase tracking-wider text-right">
      ...
      <div className="p-4 sm:p-6 bg-white/5 border-t border-white/5 flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="flex items-center gap-4">
          <button type="button" disabled aria-label={t("costBreakdown.downloadPdf")} className="flex items-center gap-2 px-4 py-2.5 bg-white/5 rounded-lg border border-white/5 text-xs font-bold uppercase tracking-wider text-[#666] cursor-not-allowed" title={t("costBreakdown.availableSoon")}>
            {t("costBreakdown.downloadPdf")}
          </button>
          <button type="button" disabled aria-label={t("costBreakdown.exportExcel")} className="flex items-center gap-2 px-4 py-2.5 bg-white/5 rounded-lg border border-white/5 text-xs font-bold uppercase tracking-wider text-[#666] cursor-not-allowed" title={t("costBreakdown.availableSoon")}>
            {t("costBreakdown.exportExcel")}
          </button>
        </div>
        <button type="button" disabled aria-label={t("costBreakdown.recalculate")} className="bg-[#ccff00]/20 text-black/50 px-8 py-2.5 rounded-lg font-bold text-xs uppercase tracking-wider cursor-not-allowed" title={t("costBreakdown.availableSoon")}>
